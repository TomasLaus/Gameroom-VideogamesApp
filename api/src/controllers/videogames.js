            ...
            if( videoDB.length !== 0) {
             ...
             let respApi = videoApi.map( r => {
                 return {
                     ...
                     released: r.released,
                     rating: r.rating,
                     platforms: r.platforms.map(p => p.platform.name.toUpperCase()),
...
const postVideoGame = async (req, res, next) => {
    try {
        const {name, id, description, image, released, rating, platforms, genres} = req.body;
